...
## v12 - Acceso con contraseña / Firebase Auth

La contraseña no se guarda en Firestore. La maneja Firebase Auth.

Flujo recomendado:

1. Mantén `NEXT_PUBLIC_AUTH_ENFORCED=false` mientras configuras los primeros usuarios.
2. En Vercel agrega las variables `FIREBASE_ADMIN_PROJECT_ID`, `FIREBASE_ADMIN_CLIENT_EMAIL`, `FIREBASE_ADMIN_PRIVATE_KEY` y `AUTH_SETUP_TOKEN`.
3. Entra a **Usuarios** y crea/edita el usuario Master con correo real.
4. Presiona **Crear acceso**. Para el primer usuario, la app pedirá el `AUTH_SETUP_TOKEN`.
5. Firebase crea el usuario de Auth y se envía un correo para definir contraseña.
6. Entra por `/login` con correo y contraseña.
7. Cuando todos los usuarios importantes estén conectados, cambia `NEXT_PUBLIC_AUTH_ENFORCED=true` en Vercel.

Campos nuevos en `platformUsers`:

- `authUid`: UID del usuario en Firebase Auth.
- `inviteStatus`: `pending_auth`, `auth_created`, `reset_sent`, `active` o `disabled`.
- `passwordResetSentAt`: fecha del último correo de contraseña.
- `lastLoginAt`: último acceso registrado.

Variables necesarias para crear usuarios desde la plataforma:
...
En `FIREBASE_ADMIN_PRIVATE_KEY`, conserva los saltos de línea como `\n` si la pegas en Vercel.
