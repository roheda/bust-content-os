...
export default function ProductionsPage(){
  ...
  const editingOriginal = editing?.id ? productions.find(item=>item.id===editing.id) : null;
  const editingDirty = Boolean(editing && editingOriginal && JSON.stringify(editing)!==JSON.stringify(editingOriginal));
  ...
  return <AppShell active="Producciones">
    ...
    {brief && <AppModal onClose={()=>setBrief(null)} cardStyle={{width:"min(1200px,96vw)"}}>
      <div className="brief-actions">
        <button className="btn blue" onClick={()=>window.print()}>Imprimir / Guardar PDF tamaño hoja</button>
        <button className="btn red" data-modal-close>Cerrar</button>
      </div>
      <ProductionBrief production={brief} requests={requests}/>
    </AppModal>}

    {editing && <AppModal onClose={()=>setEditing(null)} dirty={editingDirty}>
    ...
    {showModal && <AppModal onClose={closeNewProduction} dirty={formDirty}>
      ...
      <div style={{display:"flex",gap:12,marginTop:16}}><button className="btn blue" onClick={submit}>Crear producción</button><button className="btn red" data-modal-close>Cerrar</button></div>
    </AppModal>}
  </AppShell>
}
...
function PreviewModal({file,onClose}:{file:ReferenceFile;onClose:()=>void}){
  return <AppModal onClose={onClose} cardClassName="preview-box">
    <div className="preview-actions"><strong>{file.name}</strong><button className="btn red" data-modal-close>Cerrar</button></div>
