...
export default function ClientBrandBrainPage(){
  ...
  const [form,setForm]=useState({
    ...
    copyTone:"",allowedWords:"",forbiddenWords:"",allowedEmojis:"",preferredCtas:"",baseHashtags:"",copySpecialInstructions:"",approvedExamples:"",copyNeverDo:""
  });
  ...
  async function cleanClientData(){
    ...
    try{
      ...
      alert(error instanceof Error ? error.message : "No se pudo limpiar la información del cliente.");
    }finally{
      setCleaning(false);
    }
  }
  ...
  return <AppShell active="Clientes">
    ...
    <section className="brandbrain-layout">
      <div className="brandbrain-card">
        ...
        <div className="brandbrain-form">
          ...
          <div className="copy-rules-box">
            ...
            <p className="mini">Estas reglas alimentan Contenidos, Creador de Solicitudes y la IA de copy para que no dependa de escribir algo primero.</p>
            <div className="client-profile-grid"><div className="field"><label>Tono específico de copy</label><input value={form.copyTone} onChange={e=>set("copyTone",e.target.value)} placeholder="Ej. cercano, premium, directo"/></div><div className="field"><label>Emojis permitidos</label><input value={form.allowedEmojis} onChange={e=>set("allowedEmojis",e.target.value)} placeholder="Separados por coma"/></div></div>
            <div className="client-profile-grid"><div className="field"><label>Palabras que sí se pueden usar</label><textarea value={form.allowedWords} onChange={e=>set("allowedWords",e.target.value)} placeholder="Separadas por coma"/></div><div className="field"><label>Palabras prohibidas</label><textarea value={form.forbiddenWords} onChange={e=>set("forbiddenWords",e.target.value)} placeholder="Separadas por coma"/></div></div>
            <div className="client-profile-grid"><div className="field"><label>CTAs preferidos</label><textarea value={form.preferredCtas} onChange={e=>set("preferredCtas",e.target.value)} placeholder="Agenda tu cita, Escríbenos, Cotiza hoy"/></div><div className="field"><label>Hashtags base</label><textarea value={form.baseHashtags} onChange={e=>set("baseHashtags",e.target.value)} placeholder="#marca, #servicio"/></div></div>
            <div className="field"><label>Indicaciones especiales</label><textarea value={form.copySpecialInstructions} onChange={e=>set("copySpecialInstructions",e.target.value)} placeholder="Reglas editoriales, estructura, longitud, tono comercial."/></div>
            <div className="field"><label>Ejemplos de copys aprobados</label><textarea value={form.approvedExamples} onChange={e=>set("approvedExamples",e.target.value)} placeholder="Pega ejemplos que sí representan el estilo del cliente."/></div>
      ...
      <aside className="brandbrain-card">
        ...
        <div className="client-cleaner-box">
