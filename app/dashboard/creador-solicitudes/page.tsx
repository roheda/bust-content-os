...
export default function CreatorPage(){
  ...
  function setBatchDueDateSafe(value:string){
    if(isWeekend(value)){
      alert("La fecha límite del lote debe ser un día hábil.");
      return;
    }
    setBatchDueDate(value);
  }
  ...
  function hydrate(req: ContentRequest, source:string): ContentRequest{
    return {
      ...
      status: req.copyStatus === "pendiente" || req.copyStatus === "en_proceso" || !(req.copyIn || "").trim() ? "pendiente_copy" : req.requiresProduction ? "pendiente_produccion" : "lista_asignacion",
      copyStatus: req.copyStatus || ((req.copyIn || "").trim() ? "listo_para_revision" : "pendiente"),
      batchDueDate,
  ...
  function generateManualBlanks(){
    if(!client?.id)return alert("Selecciona cliente");
    if(!startDate)return alert("Define la fecha de primera publicación.");
    if(!draftName)setDraftName(`${client.name} · Lote ${new Date().toLocaleDateString("es-MX")}`);
    const typeList=split(types), goalList=split(goals);
    const generated = Array.from({length:Math.max(1,manualCount)}).map((_,i)=>{
      const contentType=typeList[i%typeList.length]||manual.contentType||"Post";
      const objective=goalList[i%goalList.length]||manual.objective||"Ventas";
      const suggestedArea = ["Reel","TikTok","Foto"].includes(contentType) ? "Audiovisual" : "Diseño";
      return hydrate({
        ...emptyRequest,
        contentType,
        ...
        cta:"",
        publishDate:addDays(startDate,i*interval),
        source:"manual_blank",
        status:"pendiente_copy"
      },"manual_blank");
    });
    setItems([...items,...generated]);
  }
  ...
  function validateBatch(){
    ...
    const errors = items.map((item,index)=>({index,error:validateCreatorItem(item,{strict:item.source!=="manual_blank"})})).filter(x=>x.error);
    if(errors.length){
  ...
  return <AppShell active="Creador de Solicitudes">
    ...
    <div className="batch-bar">
      ...
      <div className="field" style={{margin:0}}>
        ...
        <input type="date" value={batchDueDate} onChange={e=>setBatchDueDateSafe(e.target.value)}/>
      </div>
      ...
      <div className="grid">
        <div className="card creator-card">
          ...
          <div className="creator-tabs">
            <button className={creatorMode==="ia"?"active":""} onClick={()=>setCreatorMode("ia")}>Modo IA</button>
            <button className={creatorMode==="manual"?"active":""} onClick={()=>setCreatorMode("manual")}>Modo Manual</button>
          </div>

          {creatorMode==="ia" ? <div className="mode-panel">
            <div className="mode-note">
              <strong>Modo IA</strong>
              <span>Genera propuestas automáticamente con base en el brief, reglas del cliente y configuración del lote.</span>
            </div>
            <div className="form-grid">
              ...
              <div className="field"><label>Primera publicación</label><input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/></div>
          ...
          </div> : <div className="mode-panel">
            <div className="mode-note">
              <strong>Modo Manual</strong>
              <span>Crea solicitudes en blanco para que el equipo las llene manualmente. No se genera copy ni idea con IA.</span>
            </div>
            <div className="form-grid">
              <div className="field"><label>Cuántas solicitudes</label><input type="number" value={manualCount} onChange={e=>setManualCount(Number(e.target.value))}/></div>
              <div className="field"><label>Primera publicación</label><input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/></div>
              <div className="field"><label>Cada cuántos días</label><input type="number" value={interval} onChange={e=>setInterval(Number(e.target.value))}/></div>
              <div className="field"><label>Tipos</label><input value={types} onChange={e=>setTypes(e.target.value)} placeholder="Reel,Carrusel,Post"/></div>
              <div className="field"><label>Objetivos</label><input value={goals} onChange={e=>setGoals(e.target.value)} placeholder="Ventas,Awareness"/></div>
              <div className="field"><label>Área default</label><select value={manual.suggestedArea} onChange={e=>setManualField("suggestedArea",e.target.value)}>{areas.map(x=><option key={x}>{x}</option>)}</select></div>
            </div>
            <button className="btn blue" onClick={generateManualBlanks}>Crear solicitudes en blanco</button>
          ...
          <div className="table-wrap">
            <table className="table">
              ...
              <tbody>{items.map((item,index)=>{
                const error = validateCreatorItem(item,{strict:item.source!=="manual_blank"});
                return <tr key={index}>
                  <td>
