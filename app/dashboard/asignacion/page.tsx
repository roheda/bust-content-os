...
const organizationAreaByName = new Map(
  organizationTeam.map((user) => [
    normalizePersonKey(user.name),
    normalizeAssignableArea(user.area),
  ]),
);
...
function normalizeAssignableArea(value = "") {
  const normalized = value
    ...
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
  if (normalized.includes("diseno") || normalized.includes("diseño"))
    return "Diseño";
  if (
...
export default function AssignmentPage() {
  ...
  const orphanRequests = useMemo(
    () =>
      brands.length
        ...
        : [],
    [items, brands.length, activeClientIds],
  );
  const visibleBaseItems = useMemo(
    () =>
      brands.length
        ? items.filter(
            (item) => !item.clientId || activeClientIds.has(item.clientId),
          )
        : items,
    [items, brands.length, activeClientIds],
  );
  ...
  const teamOptionsByArea = useMemo(() => {
    ...
    organizationTeam
      .filter((user) =>
        assignableProductionAreas.includes(normalizeAssignableArea(user.area)),
      )
      .forEach((user) => {
  ...
  function getAreaForItem(item: Partial<ContentRequest>) {
    return (
      normalizeAssignableArea(item.assignedArea || item.suggestedArea || "") ||
      "Diseño"
    );
  }
  ...
  function sortValue(item: ContentRequest, key: string) {
    ...
    if (key === "request")
      return `${item.contentType} ${item.objective} ${item.creativeIdea}`;
    if (key === "dueDate")
      return item.batchDueDate || item.dueDate || item.publishDate || "";
  ...
  const batchOptions = useMemo(() => {
    ...
    visibleBaseItems
      ...
      .forEach((item) =>
        map.set(item.batchId || "sin-lote", item.batchName || "Sin lote"),
      );
    return Array.from(map.entries())
      .map(([id, name]) => ({ id, name }))
      .sort((a, b) => a.name.localeCompare(b.name, "es"));
  }, [visibleBaseItems, client]);
      ...
      visibleBaseItems
        .filter((item) => {
          ...
          return (
            ...
            (batchFilter === "all" ||
              (item.batchId || "sin-lote") === batchFilter) &&
            (area === "all" ||
              item.suggestedArea === area ||
              item.assignedArea === area) &&
            statusMatchesFilter(item, status)
          );
        })
        .sort(
          (a, b) =>
            String(sortValue(a, sort.key)).localeCompare(
              String(sortValue(b, sort.key)),
              "es",
              { numeric: true },
  ...
  async function update(id: string, data: Partial<ContentRequest>) {
    ...
    if (current && touchesAssignment && !(data as any).comments) {
      payload.comments = [
        ...
        assignmentLog(
          current,
          `Ajuste de asignación. Responsable: ${data.assignedTo ?? current.assignedTo ?? "Sin responsable"}. Área: ${data.assignedArea ?? current.assignedArea ?? current.suggestedArea ?? "Sin área"}. Prioridad: ${data.priority ?? current.priority ?? "Media"}.`,
          "Asignación",
        ),
      ];
    }
  ...
  function buildAssignmentPayload(
    ...
    const plan = getOperationalPlan(
      { ...item, assignedTo: assignee, assignedArea: areaValue },
      costRules,
      clientOverrides,
    );
    const schedule = planWorkDateForAssignment(
      item,
      items,
      teamCapacities,
      costRules,
    ...
    const capacityTone = getCapacityTone(
      schedule.projectedLoad,
      schedule.capacity,
    );
    return {
      ...
      operationalWeight: 1,
      operationalRisk:
        schedule.overflow || capacityTone.tone === "red"
          ? "red"
          : capacityTone.tone === "orange"
            ? "orange"
            : capacityTone.tone === "yellow"
              ? "yellow"
              : "green",
      carriedOver: false,
      carriedOverDays: 0,
  ...
  async function assignSelected() {
    ...
    if (!assignable.length) {
      alert(
        "Ninguna de las solicitudes seleccionadas está lista para asignarse. Las piezas que requieren producción deben tener material listo antes de pasar a diseño/audiovisual.",
      );
      return;
    }
    await Promise.all(
  ...
  async function cleanupOrphanRequests() {
    ...
    if (
      !confirm(
        `Se eliminarán ${orphanRequests.length} solicitud(es) de clientes que ya no existen. ¿Continuar?`,
      )
    )
      return;
    await Promise.all(
      orphanRequests.map((item) =>
        deleteRequest(
          item.id!,
          "Cliente eliminado / limpieza de basura de pruebas",
        ),
      ),
    );
    await load();
    alert("Solicitudes huérfanas eliminadas de Asignación.");
  }
  ...
  return (
    <AppShell active="Asignación">
      ...
      <section className="grid kpis">
        {[
          ["Total", String(visibleBaseItems.length)],
          ["Filtradas", String(filtered.length)],
          ["Seleccionadas", String(selected.length)],
          ["Listas", String(visibleBaseItems.filter(canAssignRequest).length)],
          [
            "Pend. producción",
            String(
              visibleBaseItems.filter(
                (x) => getOperationalStatus(x) === "pendiente_produccion",
              ).length,
            ),
          ],
          [
            ...
            String(
              visibleBaseItems.filter(
                (x) => getOperationalStatus(x) === "bloqueada",
              ).length,
            ),
          ],
          [
            "Asignadas",
            String(
              visibleBaseItems.filter((x) => x.status === "asignada").length,
            ),
          ],
        ].map(([a, b]) => (
        ...
        <div className="bulk-actions">
          ...
          {selectedBlockedCount > 0 && (
            <span className="pill amber">
              {selectedBlockedCount} no asignable(s)
            </span>
          )}
          {selectedAssignableCount > 0 && (
            <span className="pill green">
              {selectedAssignableCount} lista(s)
            </span>
          )}
          <select
        ...
        {editing && (
          <div className="card assignment-detail-card">
            ...
            <RequestDetail
              ...
              getTeamOptionsForArea={getTeamOptionsForArea}
      ...
      {rejectModal && (
        <div className="modal-backdrop">
          <div className="modal-card">
            ...
            <p className="mini">
              Usa esto cuando la solicitud ya avanzó a asignación pero necesita
              corrección. Si fue un borrador mal creado, elimínalo desde Creador
              de Solicitudes.
            </p>
            <div className="field">
              <label>Nota de rechazo</label>
              <textarea
                value={rejectNote}
                onChange={(e) => setRejectNote(e.target.value)}
                placeholder="Ej. Falta material correcto / brief incompleto / no aplica al objetivo."
              />
            </div>
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button className="btn red" onClick={() => setRejectModal(false)}>
                Cancelar
              </button>
            </div>
          </div>
      ...
      {deleteModal && (
        <div className="modal-backdrop">
          <div className="modal-card">
            ...
            <p>
              Esta acción eliminará {selected.length} solicitud(es). Para
              confirmar escribe <strong>ELIMINAR</strong>.
            </p>
            <div className="field">
...
function RequestDetail({
  ...
  const op = getOperationalStatus(item);
  const detailArea =
    draft.assignedArea || item.assignedArea || item.suggestedArea || "Diseño";
  const detailTeamOptions = getTeamOptionsForArea(detailArea);
  const validDetailAssignee = detailTeamOptions.includes(
  ...
  return (
    <div className="assignment-detail-scroll">
      <div className="detail-hero">
        ...
        <div className="detail-meta">
          ...
          <span className="pill blue">
            {item.suggestedArea || item.assignedArea || "Sin área"}
          </span>
          <span
            className={item.requiresProduction ? "pill orange" : "pill green"}
          >
        ...
        {item.rejectionNote && (
          <div className="reject-note">Rebotada: {item.rejectionNote}</div>
        )}
        <div className="form-grid">
          ...
          <div className="field">
            ...
            <input
              value={item.publishDate || item.clientDueDate || "Sin fecha"}
              disabled
            />
          </div>
          <div className="field">
      ...
      <div className="detail-section">
        ...
        {!referenceLinks.length && !(item.referenceFiles || []).length && (
          <p className="mini">Sin referencias.</p>
        )}
      </div>
      ...
      {item.requiresProduction && (
        <div className="detail-section">
          ...
          <div className="detail-copy">
            {item.productionNotes || "Sin notas de producción"}
          </div>
        </div>
      )}
      ...
      <div className="assignment-actions">
        ...
        <div className="danger-zone">
          ...
          <p className="mini">
            Úsalo cuando el brief ya llegó a asignación pero necesita
            corrección. En creación, lo correcto es editar o eliminar el
            borrador.
          </p>
          <textarea
