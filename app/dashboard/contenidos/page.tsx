...
export default function ContenidosPage() {
  ...
  function normalizedStatus(item: ContentRequest) {
    ...
    if (
      item.copyStatus === "aprobado" ||
      item.copyStatus === "listo_para_revision"
    )
      return "copy_listo";
    if (
      item.copyStatus === "pendiente" ||
      item.copyStatus === "en_proceso" ||
      item.status === "pendiente_copy"
    )
      return "pendiente_copy";
    const savedCopy = (item.copyOut ?? "").trim();
    return savedCopy ? "copy_listo" : "pendiente_copy";
  }
  function statusLabel(value: string) {
    return value === "finalizada"
      ? "Finalizada"
      : value === "copy_listo"
        ? "Copy listo"
        : "Pendiente de copy";
  ...
  function buyerPersonaContext(item: ContentRequest) {
    ...
    return [
      ...
      persona.contentAngles
        ? `Ángulos de contenido: ${persona.contentAngles}`
        : "",
    ]
      .filter(Boolean)
      .join(" | ");
  ...
  const readyForContent = useMemo(
    () =>
      requests.filter((x) => {
        ...
        return (
          x.status === "aprobada_pendiente_copyout" ||
          x.status === "aprobada_pendiente_contenidos" ||
          (x.approvalStatus === "aprobada" && x.status !== "finalizada")
        );
      }),
    [requests],
  );
  ...
  const clientOptions = useMemo(() => {
    ...
    [
      ...readyForContent,
      ...requests.filter((x) => x.status === "finalizada"),
    ].forEach((x) =>
    ...
    () =>
      readyForContent
        .filter((x) => {
          ...
          const haystack =
            `${x.clientName} ${x.batchName} ${taskTitle(x)} ${typeLabel(x)} ${platformsLabel(x)} ${x.copyOut || ""} ${x.copyIn || ""} ${x.finalPostLink || ""}`.toLowerCase();
          return (
            ...
            (statusFilter === "all" ||
              statusFilter === status ||
              (statusFilter === "pending" && status === "pendiente_copy")) &&
            (!search.trim() || haystack.includes(search.trim().toLowerCase()))
          );
        })
        .sort((a, b) => {
          const result = sortValue(a).localeCompare(sortValue(b), "es", {
            numeric: true,
            sensitivity: "base",
          });
  ...
  const finalized = useMemo(
    () =>
      requests
        .filter((x) => {
          const haystack =
            `${x.clientName} ${x.batchName} ${taskTitle(x)} ${typeLabel(x)} ${platformsLabel(x)} ${x.copyOut || ""} ${x.finalPostLink || ""}`.toLowerCase();
          return (
            x.status === "finalizada" &&
            (clientFilter === "all" || x.clientId === clientFilter) &&
            (batchFilter === "all" ||
  ...
  async function requestImprovedCopyOut(item: ContentRequest) {
    ...
    const approvedCopies = approvedCopyExamples(item)
      .map((row: any) => row.copyOut || String(row))
      .filter(Boolean);
    ...
    try {
      ...
      alert(
        error instanceof Error
          ? error.message
          : "No se pudo generar copy con IA.",
      );
    } finally {
  ...
  async function saveCopy(
    ...
    if (finish) {
      comments.push({
        ...
        target: "Interno",
        body: "Copy final guardado y contenido finalizado.",
        mentions: [],
        createdAt: new Date().toISOString(),
      });
      const temporaryReferenceFiles = (item.referenceFiles || []).filter(
        (file) => file.temporary || file.storagePath,
      );
      ...
      await updateRequest(item.id, {
        ...
        referenceFiles: (item.referenceFiles || []).filter(
          (file) => !temporaryReferenceFiles.includes(file),
        ),
        comments,
      });
    } else {
      comments.push({
        ...
        target: "Interno",
        body: markReady
          ? "Copy marcado como listo para revisión."
          : "Copy guardado en proceso.",
        mentions: [],
        createdAt: new Date().toISOString(),
      });
      await updateRequest(item.id, {
        copyOut,
        copyStatus: markReady ? "listo_para_revision" : "en_proceso",
        status: markReady ? "aprobada_pendiente_copyout" : "pendiente_copy",
        comments,
      });
    }
  ...
  async function finishOne(item: ContentRequest) {
    await saveCopy(item, true);
    await load();
  }
  async function finishSelected() {
    ...
    if (!rows.length)
      return alert("Selecciona piezas pendientes para finalizar.");
    for (const item of rows) {
      await saveCopy(item, true);
    }
    setSelected([]);
    await load();
    alert(`Finalizadas ${rows.length} pieza(s).`);
  }
  ...
  function exportRows(rows: ContentRequest[], filename: string) {
    ...
    const csvRows = [
      ...
      ...rows.map((x) => [
        ...
        platformsLabel(x),
        taskTitle(x),
        copyOutDrafts[x.id || ""] ?? x.copyOut ?? "",
        x.finalPostLink || "",
        statusLabel(normalizedStatus(x)),
        x.assignedTo || "",
      ]),
    ];
    const csv = csvRows
  ...
  return (
    <AppShell active="Contenidos">
      <section className="hero">
        <div>
          <p className="eyebrow">Salida y publicación</p>
          <h1>Contenidos</h1>
          <p>
            Copy final, links, exportación y cierre de piezas ya aprobadas por
            Content y KAM.
          </p>
        </div>
      </section>
      ...
      <section className="grid kpis">
        {[
          [
            ...
            String(
              visibleItems.filter(
                (x) => normalizedStatus(x) === "pendiente_copy",
              ).length,
            ),
          ],
          [
            "Copy listo",
            String(
              visibleItems.filter((x) => normalizedStatus(x) === "copy_listo")
                .length,
            ),
          ],
          ["Finalizadas", String(finalized.length)],
      ...
      <section className="card">
        <div className="finalized-group-title">
          <div>
            <h3>Contenido listo para salida</h3>
            <p className="mini">
              Filtra, genera copy con IA, copia textos y exporta para subir a
              plataformas.
            </p>
          </div>
        </div>
        <div className="finalized-toolbar copyout-toolbar">
          ...
          <input
            ...
            placeholder="Buscar por tarea, copy, link..."
          />
          <button
            className="btn"
            onClick={() => {
          ...
          <button
            ...
            onClick={() =>
              exportRows(
                selectedRows.length ? selectedRows : tableItems,
                `contenidos-${new Date().toISOString().slice(0, 10)}.csv`,
              )
            }
          >
            Exportar Excel/CSV
        ...
        <div className="copyout-row header contenidos-header">
          ...
          <button type="button" onClick={() => toggleSort("copyOut")}>
            Copy final{sortLabel("copyOut")}
          </button>
        ...
        {tableItems.map((item) => (
          <div className="copyout-row contenidos-row" key={item.id}>
            <input
              type="checkbox"
            ...
            <div className="list-truncate-cell">
            ...
            <TextTooltip
              as="div"
              className="copyout-cell"
              text={
                copyOutDrafts[item.id || ""] ?? item.copyOut ?? "Sin copy final"
              }
            >
              <textarea
                rows={2}
                value={copyOutDrafts[item.id || ""] ?? item.copyOut ?? ""}
                onChange={(e) =>
                  setCopyOutDrafts({
                    ...copyOutDrafts,
              ...
              {item.status !== "finalizada" && (
                <button
                  ...
                  onClick={() => improveOne(item)}
                  disabled={improvingCopyId === item.id || bulkImproving}
                >
            ...
            <span>
              <span
                className={
                  normalizedStatus(item) === "finalizada"
                    ? "pill green"
                    : "pill"
                }
              >
        ...
        {!tableItems.length && (
          <p className="mini">No hay contenidos con esos filtros.</p>
        )}
      </section>
    </AppShell>
  );
