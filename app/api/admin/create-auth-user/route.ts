...
export async function POST(req: NextRequest) {
  try {
    ...
    const currentProfileSnap = await adminDb.collection("platformUsers").doc(platformUserId).get();
    const currentProfile = currentProfileSnap.exists ? (currentProfileSnap.data() as PlatformUserDoc) : {};
    const mergedProfile: PlatformUserDoc = { ...currentProfile, authUid: authUser.uid, email, name };
    ...
    await adminDb.collection("userAccess").doc(authUser.uid).set(buildUserAccess(authUser.uid, platformUserId, mergedProfile), { merge: true });
