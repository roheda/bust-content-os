...
function buildGeminiPrompt(prompt: string) {
  ...
  const safePrompt = trimmedPrompt.length > maxPromptLength ? trimmedPrompt.slice(0, maxPromptLength) : trimmedPrompt;
...
async function generateOneGeminiImage({
  ...
  if (!response.ok) {
    const message = typeof payload?.error?.message === "string" ? payload.error.message : `Gemini no pudo generar la imagen con ${model}.`;
  ...
  if (typeof imageBase64 !== "string" || imageBase64.length === 0) {
    ...
    throw new Error(`Gemini respondió sin imagen utilizable con ${model}.${finishReasonText}${shortResponseText}`);
  }
...
export async function POST(request: Request) {
  try {
    ...
    const referenceImages = Array.isArray(body.referenceImages) ? (body.referenceImages as ReferenceImageInput[]) : [];
    ...
    if (!result.imagesBase64.length) {
      return NextResponse.json({ error: "No se recibió ninguna imagen del proveedor." }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: errorMessage || "Error al generar la imagen." }, { status: 500 });
  }
}
